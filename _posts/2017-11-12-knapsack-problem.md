...
假设第一个CPU处理时间是$$Sum1$$，第二个CPU处理时间是$$Sum2$$，假设$$Sum1 < Sum2$$，则$$Sum1$$与$$Sum2$$越接近，则其总处理时间最小，又因为$$Sum1 + Sum2 = Sum$$，$$Sum$$是所有任务所需时间之和，则当$$Sum1$$越接近$$\frac{1}{2}Sum$$，处理时间最小。

所以可以转换为01背包问题，最大容量是$$\frac{1}{2}Sum$$，对于一个任务，可以选择处理和不处理。
